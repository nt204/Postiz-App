"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Dub = void 0;
const DUB_API_ENDPOINT = process.env.DUB_API_ENDPOINT || 'https://api.dub.co';
const DUB_SHORT_LINK_DOMAIN = process.env.DUB_SHORT_LINK_DOMAIN || 'dub.sh';
const getOptions = () => ({
    headers: {
        Authorization: `Bearer ${process.env.DUB_TOKEN}`,
        'Content-Type': 'application/json',
    },
});
class Dub {
    constructor() {
        this.shortLinkDomain = DUB_SHORT_LINK_DOMAIN;
    }
    async linksStatistics(links) {
        return Promise.all(links.map(async (link) => {
            const response = await (await fetch(`${DUB_API_ENDPOINT}/links/info?domain=${this.shortLinkDomain}&key=${link.split('/').pop()}`, getOptions())).json();
            return {
                short: link,
                original: response.url,
                clicks: response.clicks,
            };
        }));
    }
    async convertLinkToShortLink(id, link) {
        return (await (await fetch(`${DUB_API_ENDPOINT}/links`, {
            ...getOptions(),
            method: 'POST',
            body: JSON.stringify({
                url: link,
                tenantId: id,
                domain: this.shortLinkDomain,
            }),
        })).json()).shortLink;
    }
    async convertShortLinkToLink(shortLink) {
        return await (await (await fetch(`${DUB_API_ENDPOINT}/links/info?domain=${shortLink}`, getOptions())).json()).url;
    }
    async getAllLinksStatistics(id, page = 1) {
        const response = await (await fetch(`${DUB_API_ENDPOINT}/links?tenantId=${id}&page=${page}&pageSize=100`, getOptions())).json();
        const mapLinks = response.links.map((link) => ({
            short: link,
            original: response.url,
            clicks: response.clicks,
        }));
        if (mapLinks.length < 100) {
            return mapLinks;
        }
        return [...mapLinks, ...(await this.getAllLinksStatistics(id, page + 1))];
    }
}
exports.Dub = Dub;
//# sourceMappingURL=dub.js.map